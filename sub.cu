...
// Kernel for cuda_sub() for when broadcasting is not needed
...
// Kernel for cuda_sub() for when broadcasting is needed
__global__ void broadcast_sub_kernel(const float* __restrict__ A,
