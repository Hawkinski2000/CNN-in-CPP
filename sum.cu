...
__global__ void sum_all_kernel(const float* __restrict__ data, float* result, size_t size) {
    ...
    int i = blockIdx.x * blockDim.x + threadIdx.x; // Index of element in data to process
    ...
    // Parallel reduction in shared memory. The range in data to sum is halved
    // each time until the overall sum of data is accumulated in sdata[0].
    // E.g., if data contains [1, 2, 3, 4], after each iteration, sdata will
    // contain [1+3, 2+4, 3, 4], then [4+6, 6, 3, 4], where sdata[0] = 10, the
    // sum of 1 + 2 + 3 + 4.  
...
__global__ void sum_dim_kernel(const float* __restrict__ input,
    ...
    extern __shared__ size_t shared_mem[]; // array in shared memory
